...
def load_data(filename):
    ...
    with open(filename, mode='r', encoding='utf-8') as file:
        ...
        for row in reader:
            if row: # Ensures the row isn't  empty
                # Grab Column A (index 0) which contains the Arabic text
                sentences.append(row[0])
    return sentences

def tokenize(text):
    #lowercase and give only valid words
...
def build_model(sentences):
    model = {}
    unigram_counts = {} # NEW: Dictionary to track every single word's frequency
    ...
    # Find the top 3 most common words in the entire dataset
